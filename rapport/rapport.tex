...
Il est clair que dans la deuxième étape de l'algorithme, nous devons faire une boucle allant de 0 à $\frac{n}{2}-1$ pour les polynômes $R_0$ et $R_1$. Cette opération s'effectue en $O(n)$. De plus, le degré étant divisé par deux à chaque tour de récursion, on voit que la complexité de la DFT est en $O(n*log_2(n))$. Le fait de pouvoir stocker une telle somme était pratique, par exemple pour \\
...
\\ FAIRE implémentaiton Multiplication.
